/**
 * 判词字段 DDL 执行脚本（仙侠人物判词生成 Skill）
 * 读取 .env 配置，连接创作库执行 ddl-custom-verdict.sql（幂等）
 *
 * 用法: node scripts/run-ddl-custom-verdict.mjs
 * 回滚: node scripts/run-ddl-custom-verdict.mjs --rollback
 */
...
async function main() {
  ...
  const sqlFile = rollback ? 'ddl-custom-verdict-rollback.sql' : 'ddl-custom-verdict.sql';
  ...
  try {
    ...
    if (!rollback) {
      const check = await client`
        ...
        WHERE table_name = 'custom_character' AND column_name LIKE 'verdict%'
      `;
      console.log(`[ok]   判词字段 (${check.length}):`);
