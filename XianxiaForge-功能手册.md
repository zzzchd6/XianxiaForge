...
| | 2 世界观百科 | 世界库实体图鉴/宗门规制/岁时节令/文风引擎（RAG 数据源） |
...
| | 26 山河舆图 | 项目地图布点/路径连线/行程估算/世界库导入/正文地点注入/瞬移预警（v1.3 新增） |
| | 27 实体自动维护 | 章节生成后自动发现新人物/武器/功法/地点建草稿 + 老实体动态台账 + 跨章事实冲突检测（v1.3 新增，v1.5 增强） |

**规模数字**：双库 66+ 张表（仙侠世界 18 + 创作 Drizzle 48+，另含 Python 侧 plot_material_*/style_preset/domain_knowledge/benchmark 素材表）｜ 24 个 Agent 文件（含底座 BaseAgent，v1.5 新增 conflict-generator/iceberg-dialogue）｜ 34 个后端路由文件｜ 21 个前端路由页面（+双引擎工坊）｜ 30 维剧情审计 + 8 维文风审计｜ 10 大类 41 叙事方向｜ 增强能力 100% 降级兜底。
...
| 智能匹配素材 | 扫描节点文本从世界库名目录召回候选实体 | match-materials 🔒 | 模块2/18 |
...
| 大纲引导初始化 | 从世界人物库+卷大纲引导初始状态 | bootstrap | 模块2/3 |
...
| 数据库状态 | 世界库/创作库连接 + 刷新 | db-status | 模块2 |
| 方向字典暴露 | 全局方向字典查询 | direction-catalog | 模块7 |

### 3.21 自定义人物工坊

**定位**：三步向导创建自定义人物（种族与定位→性格立场→先天禀赋），创建后 AI 生成七言判词+考语。对外暴露**负数ID**，与世界库正数ID共存分流。
...
| 仙古遗珍彩蛋 ★ | 极低概率（0.1%）产出仙古遗珍稀有彩蛋（weapon-relic） | random 🔒 | — |
...
> **本次变更（2026-07-31）**：人兵功系统扩展——路边摊淘宝（batch/junkRatio/autoDisguise）+ 统一五感卡刷新服务 + 烙印系统（4预定义+自定义）+ 因果羁绊三维度自动匹配 + 重铸 + 套装道号/连携技 + 走火入魔魔改/净化 + 天命克制计算 + 仙古遗珍0.1%彩蛋 + 老数据迁移；审计新增 OOC 改文建议与稀有特质矛盾检测。前端新增 StreetMarketDialog/ScarSection/RecraftSection/器灵显示区。Schema：weapon_lore+spirit、custom_character+daoTitle/comboAbility、custom_character_relation+entityType/weaponId。
...
| 草稿确认 | 自动抽取/世界库导入的地点为 draft，确认后 official | confirm/列表徽章 | 模块27 |
| 世界库导入 | 一键导入世界库地点（≤500条，draft + 地图边缘坐标，metadata 溯源） | import-zhuxian | 模块2 |
...
| 去重三保险 | LLM 名单排除 + 应用层 Set 双保险（本项目已有 + 世界库名单） | 🔒 入库前 | 模块2 |
...
| world.ts | `/world` | 世界库实体 CRUD/蒸馏/搜索 |
...
| custom-locations.ts | `/projects/:id/custom-locations` | 山河舆图地点/连线/世界库导入/行程估算（模块26） |
...
| services/weapon-relic | 仙古遗珍彩蛋（0.1%）🔒 |
